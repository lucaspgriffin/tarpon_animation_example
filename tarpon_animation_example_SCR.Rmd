...
setwd("/Users/lucasgriffin/Box Sync/BTT.Telemetry/L.G.Workplace/SCR")
load("TarponFeb.2020.RData")

setwd("~/github/tarpon_animation_example")

Tarpon = Tarpon %>% filter(Transmitter %in% c("A69-9001-18574", "A69-9001-15888", "A69-9001-18584"))

head(Tarpon)

Tarpon = Tarpon %>% select(ID, Transmitter, UTC, Latitude, Longitude)
save(Tarpon, file = "Tarpon.RData")
...
Migration_plot = 
  ...
  geom_point(data=Tarpon %>% arrange(moday_pos), 
             aes(x=moday_pos, y=Latitude, group = Transmitter_Year, col = Year), # color each year
             alpha = 0.5) + 
  scale_x_datetime(breaks = date_breaks("1 months"),labels = date_format("%B")) + # format our date time breaks
  xlab("")+ # no x axis label
  theme_bw() +  # cleaner look
  theme(axis.text.x = element_text(angle = 35, hjust = 1, size = 12)) + # format our x axis text
...
Migration_plot_mod = 
  ...
  scale_x_datetime(breaks = date_breaks("1 months"),labels = date_format("%b")) + # format our date time breaks (this time abbreviated)
  xlab("")+ # no x axis label
  theme_bw() +  # cleaner look
  theme(axis.text.x = element_text(angle = 35, hjust = 1, size = 12),
        legend.position = "right") + # format our x axis text and legend position
  facet_grid(~Transmitter) # plot each transmitter individually
